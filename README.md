...
迁移行为：`workspace.sqlite3` 用 `PRAGMA user_version` 版本化，`WorkspaceStore._initialize()` 在打开时按版本增量建表（当前 `VERSION = 3`）。**只支持向上迁移**：若文件版本高于当前代码版本会直接抛错，不做降级。项目注册表带 `version` 字段，版本不匹配同样拒绝读取。删除数据请直接删文件（会话例外：界面右键或 `DELETE /api/sessions/{id}` 即可，消息与事件由外键级联一并清掉）；**删除项目注册关系不会删除项目目录**（`DELETE /api/projects/{id}` 只改元数据）。
