...
- **Skill（任务规范）**：只读规范放 `<用户目录>/skills/<名称>/SKILL.md` 或项目 `.routivus/skills/` 下即被自动发现；索引注入 system prompt（`/skill enable|disable` 后下一轮生效），正文由模型按需调用 `load_skill` 工具加载，参考资料受路径白名单与字数上限约束。配置页有 Skill 列表与启停（`GET /api/skills`、`POST /api/skills/{name}/enable|disable`），会话内可用 `/skill list|load|enable|disable`。
...
- 配置页可编辑（Provider 增删改 / Key / 模型列表 / 四档 / SmartRouter 开关，走 `/api/config`）+ Skill 列表与启停（走 `/api/skills`，仅用户级与内置；**项目级 Skill 依赖项目上下文，需在项目会话里用 `/skill list` 查看**）；Memory 条目暂无接口，页面未展示。
