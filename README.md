...
- **`/team` 没有断点续跑**（`/team resume` 与它的 `--write-scope` 已移除）：任务一旦中断就是终态，只能重新 `/team <任务>` 发起。Reviewer 说"无法确定修复范围"时任务**直接判失败**（`repair_scope_missing` / `repair_scope_unsafe` / `review_execution_failed`），不再挂起等人补范围，也不会留下永不解除的 waiting 状态。
