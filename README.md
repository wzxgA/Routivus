...
这两个事件都会落进 `events` 表，并随会话快照的 `replay` 回放：**审批卡是时间线条目**——请求时插在发生的那个位置、应答后原地定格成终态，刷新 / 重连后仍在原位（此前它只活在内存里，答完即无痕）。`approval.resolved` 的**唯一**来源是 `ApprovalBridge`；agent 内部的审批决策通知不再广播成同名事件（它没有 `approval_id`，客户端按 id 匹配不上就会无条件清空当前卡——并行 worker 时表现为"A 批完，B 正等着的卡突然消失"）。
...
- **两级导航**：全局态（首页 / 笔记 / 配置）与项目态（会话 / 笔记）由 hash 路由驱动，刷新后按 URL 恢复项目、会话与视图；项目内无会话时自动创建首个会话。**会话可重命名 / 可删除**：左侧会话项右键 →「重命名会话」（只改显示名，`PATCH /api/sessions/{id}`，标题上限 200 字）或「删除会话」，删除确认后连同它的消息与事件一起从数据库删除（`DELETE /api/sessions/{id}`，外键级联）；**运行中的会话会被拒绝**（`409 session_busy`，先停止再删）；删掉地址栏指向的那个会话时，界面自动切到列表里的下一个，最后一个也删掉时会重新自动建一个新会话。
...
- **`@` 文件引用**：输入框敲 `@` 触发**路径补全**（`↑↓` 选、`Tab` 引用、`Esc` 关；与 `/` 命令补全互斥，同一时刻只开一个），候选按前缀搜当前项目（`GET /api/projects/{id}/files/search`：只读、只匹配路径不读内容、忽略 `IGNORED_DIRS`、上限 20 条；空前缀只给根下一层，不做递归）。插入的是**项目内相对路径文本**，既不是文件内容也不是附件——输入框里 `@路径` 带底纹高亮（覆盖层只画背景，光标 / 选区 / 输入法仍是原生 textarea 行为），框外另有一行「已引用」chip，`×` 一键摘除；**chip 与高亮都由文本派生**，手改文本会自动同步，不存在"列表和正文对不上"。**落库与消息流仍是用户原文**，只有发给模型的那一份多一段极短提示（`[用户提到的项目内文件]` + 每条路径标注存在 / 不存在 / 目录 + 一句"需要时用 read_file 查看内容"）——省掉"猜路径"这一轮，又不把大文件硬塞进上下文。标点即边界（`@a.ts、@b.ts` 算两个引用），支持中文路径；不支持含空格的路径（`@"my file.ts"`）、通配符与跨项目引用，`@a.py:42` 会退化成引用 `a.py`。
...
- 会话断线重连以服务端 `session.snapshot` 重建：文本（含思考段）来自 messages 表（**最近 500 条**，更早的需要翻 REST 分页），**卡片（命令 / 工具 / 计划 / 团队 / 换档 / 审批）来自快照带回的 `replay` 事件回放**（按类型取最近 1000 张卡片，流式增量不占窗口），两者按时间戳归并，顺序与在线一致（换档卡由回放里的 `router.updated` 重建，冷渲染不重播动画）；仍挂起的审批 / 计划审阅由 `pending` 字段恢复，重连后可直接继续应答。**行为变更**：`message.completed` 自 07 起只作"本轮结束"信号，不再携带整段正文（正文由 `message.segment` 逐段下发）；消费消息流的脚本需按此适配。服务进程重启后内存桥不再存在，待决交互无法恢复（计划审阅的"重新规划"同样依赖内存桥）。
...
- **信息侧栏 Session**：拆成「会话默认」（手动配置的 provider/model）与「本轮实际」（档位徽标 + 依据 + 是否真的换了模型 + 耗时），不再让同一个 key 一会儿是配置、一会儿是结果；档位徽标同样取档位色。**使用率 = 下一次请求的预估输入 ÷ 压缩触发线**（后端每步随 `session.usage` 下发这两个数），回答的是"离自动压缩还有多远"；旁边的「已用（累计）」是整个会话所有请求的合计——每一轮都要把历史重发一遍，它必然远大于当前上下文，**不能**拿它除以窗口（那样会永远显示接近 100%，且与是否要压缩毫无关系）。
- **信息侧栏 Team**（方案 16）：常驻展示**最近一次**团队任务——主任务与状态徽标、Worker 清单（角色标签 + 逐任务失败原因分类）、当前批次、完成进度条。`needs_scope` 的任务与可续跑的收尾态只做**指路**（按钮在消息流的团队卡上），侧栏不复制任何操作控件：那边是带勾选状态的表单，复制一份就是两处状态机，迟早一个亮一个不亮。数据全部来自既有的 `team.updated` 事件（与消息流卡片同一套归约），**零后端改动**；一个会话多次 `/team` 侧栏只显示最近一次，历史看消息流。状态文案与任务标记与 `TeamCard` 共用一份（`utils/teamStatus.ts`），避免同一任务两处叫法不同。
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `context_window` | `128000` | provider 默认窗口（token），用于上下文预算（自动压缩的触发线按它算） |
...
**窗口是模型级属性**，所以同一 provider 下不同模型可以有自己的值；换模型（`/model`）或智能路由换档后，界面上的窗口与压缩触发线会随之变化，这是预期行为。
