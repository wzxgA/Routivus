...
class ApprovalBridge:
    ...
    async def request(self, tool_name: str, level: str, args: dict) -> ApprovalDecision:
        ...
        self._pending = _Pending(item_id=item_id, kind="approval", tool_name=tool_name, future=future)
    ...
    async def ask(self, ask: AskRequest) -> dict[str, str] | None:
        ...
        future: asyncio.Future = asyncio.get_running_loop().create_future()
        self._pending = _Pending(item_id=item_id, kind="ask", tool_name="ask_user", future=future)
        await self._set_waiting(True)
        await self._emit(
            "approval.requested",
            {
